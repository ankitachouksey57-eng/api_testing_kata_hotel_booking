...
public class HotelBooking_Create {
    ...
    public void theUserBooksRoomWithBookingDetails(final DataTable dataTable) {
        ...
        for (Map<String, String> row : dataTable.asMaps(String.class, String.class)) {
            ...
            context.response = context.requestSetup().cookie("token", tokenValue).body(requestBody.toString()).when().post(context.session.get("endpoint").toString());
            System.out.println(" response body after booking creation >> "+context.response.body().asString());
        }
    }
    ...
    public void theUserSendsPOSTRequestWithTheInvalidBookingDetails(final DataTable dataTable) {
        for (Map<String, String> row : dataTable.asMaps(String.class, String.class)) {
            final int roomId = Integer.parseInt(generateRandomRoomId());
            requestBody = createBookingRequestBody(row, roomId);
            context.response = context.requestSetup().body(requestBody.toString()).when().post(context.session.get("endpoint").toString());
            System.out.println(" response body after booking creation >> "+context.response.body().asString());
            LOG.info("Booking has failed: " + context.response.toString());
        }
    }
    ...
    public void theUserShouldSeeTheResponseWithIncorrectField(final String errorMessage) {
        final String actualErrorMessage = context.response.jsonPath().getString("errors");
